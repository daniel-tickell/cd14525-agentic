...
class AugmentedPromptAgent:
    def __init__(self, openai_api_key, persona):
        ...
        # Done: 1 - Create an attribute for the agent's persona
    ...
    def respond(self, input_text):
        ...
        # Done: 4 - Return only the textual content of the response, not the full JSON payload.
        return  response.choices[0].message.content
...
class KnowledgeAugmentedPromptAgent:
    def __init__(self, openai_api_key, persona, knowledge):
        ...
        self.persona = persona
        # Done: 1 - Create an attribute to store the agent's knowledge.
        self.openai_api_key = openai_api_key
    ...
    def respond(self, input_text):
        ...
        # Done: 2 - Construct a system message including: (moved out of response)
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                # Done: 3 - Add a system prompt instructing the agent to assume the defined persona and explicitly forget previous context.
                {"role": "system", "content": system_prompt},
                # Done: 3 - Add the user's input prompt here as a user message.
                {"role": "user", "content": input_text}
            ],
...
class EvaluationAgent:
    ...
    def __init__(self, openai_api_key, persona, evaluation_criteria, worker_agent, max_interactions):
        ...
        # Done: 1 - Declare class attributes here
        self.openai_api_key = openai_api_key
        self.persona = persona
    ...
    def evaluate(self, initial_prompt):
        ...
        # Done: 2 - Set loop to iterate up to the maximum number of interactions:
        for i in range(self.max_interactions):
            ...
            eval_prompt = (
                ...
                # Done: 4 - Insert evaluation criteria here
                f"Meet this criteria: {self.evaluation_criteria}\n" 
                f"Respond Yes or No, and the reason why it does or doesn't meet the criteria."
            )
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                # Done: 5 - Define the message structure sent to the LLM for evaluation (use temperature=0)
                messages=[
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    #messages= Done: 6 - Define the message structure sent to the LLM to generate correction instructions (use temperature=0)
                    messages=[
                        {"role": "system", "content": self.persona},
...
class RoutingAgent():
    ...
    def __init__(self, openai_api_key, agents):
        # Initialize the agent with given attributes
        self.openai_api_key = openai_api_key
        # Done: 1 - Define an attribute to hold the agents, call it agents
        self.agents = agents

    def get_embedding(self, text):
        client = OpenAI(base_url="https://openai.vocareum.com/v1",api_key=self.openai_api_key)
        # Done: 2 - Write code to calculate the embedding of the text using the text-embedding-3-large model
        response = client.embeddings.create(
            input=[text],
            model="text-embedding-3-large"
        )
    ...
    # Done: 3 - Define a method to route user prompts to the appropriate agent
    def route(self, user_prompt):
        # Done: 4 - Compute the embedding of the user input prompt
        input_emb = self.get_embedding(user_prompt)
        best_agent = None
        best_score = -1

        for agent in self.agents:
            # Done: 5 - Compute the embedding of the agent description
            ...
            # Done: 6 - Add logic to select the best agent based on the similarity score between the user prompt and the agent descriptions
...
class ActionPlanningAgent:
    ...
    def __init__(self, openai_api_key, knowledge):
        # Done: 1 - Initialize the agent attributes here
        self.openai_api_key = openai_api_key
        self.knowledge = knowledge

    def extract_steps_from_prompt(self, prompt):

        # Done: 2 - Instantiate the OpenAI client using the provided API key
        client = OpenAI(base_url = "https://openai.vocareum.com/v1", api_key=self.openai_api_key)
        # Done: 3 - Call the OpenAI API to get a response from the "gpt-3.5-turbo" model.
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "from the user prompt the steps requested to complete the action "
                        "the user is asking for. You return the steps as a list. Only return "
         ...
         # Done: 4 - Extract the response text from the OpenAI API response
